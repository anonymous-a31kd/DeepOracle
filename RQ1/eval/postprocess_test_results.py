...
import pandas as pd
import tqdm
from .gen_tests_from_metadata import TogaGenerator, NaiveGenerator, TogsGenerator
...
if __name__ == "__main__":
    ...
    if method == "toga" or method == "togll" :
        generator = TogaGenerator()
    elif method == "naive":
        generator = NaiveGenerator()
    elif method == "togs":
        generator = TogsGenerator()
    oracle_pred_file = "oracle_preds.csv" if method == "toga" or method == "togs" or method == "togll" else "naive_oracle_preds.csv"
    ...
    # do sanity check
    for i, (mrow, urow) in enumerate(zip(metas[["project", "bug_num"]].itertuples(), 
