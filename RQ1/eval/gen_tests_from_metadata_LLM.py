...
assert_re = re.compile(r"assert\w*\s*\((.*)\)")

def get_prefix(test):
    open_curly = test.find('{')
    if open_curly != -1 and 'throws ' not in test[:open_curly]:
        test = test[:open_curly] + ' throws Exception ' + test[open_curly:]


    test = re.sub(r'^\s*//.*$', '', test, flags=re.MULTILINE)

    try_catch_re = re.compile(
        r'try\s*{(?P<try_body>.*?)}\s*catch\s*\([^)]*\)\s*{.*?}',
        re.DOTALL
    )

    fail_stmt_re = re.compile(r'fail\s*\(.*?\)\s*;', re.DOTALL)
    assert_re = re.compile(r'assert\w*\s*\((.*?)\);', re.DOTALL)


    def clean_try_blocks(code: str) -> str:
        result = ""
        pos = 0
        for match in try_catch_re.finditer(code):
            start, end = match.span()
            try_body = match.group("try_body")
            try_clean = fail_stmt_re.split(try_body)[0].strip()
            result += code[pos:start] + try_clean + "\n"
            pos = end
        result += code[pos:]
        return result

    test = clean_try_blocks(test)

    m_assert = assert_re.search(test)
    if m_assert:
        return test[:m_assert.start()]
...
def gen_test_from_meta(test_harness, corpus_path, orig_corpus, project, generator, bug, test_id, full_test_name, test_case):
    full_class_name = full_test_name.split('_LLMTest::')[0]
    package_name = '.'.join(full_class_name.split('.')[:-1])
    class_name = full_class_name.split('.')[-1]
    
    with open(f'{test_harness}/LLMTest.java') as f:
        test_harness_template = f.read()
                
    with open(f'{test_harness}/LLMTest_scaffolding.java') as f:
        scaffolding_template = f.read()

    test_case_dir = f'{corpus_path}/generated_d4j_tests/{project}/{generator}/{bug}/{test_id}/'
    orig_test_dir = f'{orig_corpus}/generated/{project}/llm/{bug}/'

    package_path = package_name.replace('.', '/')
    package_base_dir = package_path.split('/')[0]

    orig_test_file = orig_test_dir + package_path + f'/{class_name}_LLMTest.java'
    imports = set(get_imports(orig_test_file))
    harness_imports = set(get_imports(f'{args.test_harness}/LLMTest.java'))
    ...
    filled_scaffolding = filled_scaffolding.replace('{TEST_IMPORTS}', ' ')
    filled_scaffolding = filled_scaffolding.replace('{TEST_RELATIVE}', ' ')

    run(f'rm -r {test_case_dir}'.split(), capture_output=True)
    os.makedirs(test_case_dir + package_path)

    testcase_outfile = test_case_dir + package_path + f'/{class_name}_LLMTest.java'
    with open(testcase_outfile, 'w') as f:
        f.write(testcase_filled_harness)

    with open(test_case_dir + package_path + f'/{class_name}_LLMTest_scaffolding.java', 'w') as f:
...
class TogaGenerator(Generator):
    def generate(self, test_prefix, except_pred: bool, assert_pred: str, *args, **kwargs) -> str:
        ...
        # remove assert
        tmp_test = get_prefix(test_prefix) + '\n}'
        test = '@Test\n' + get_prefix(tmp_test)
...
class FilterGenerator(Generator):
    def generate(self, test_prefix, except_pred: bool, assert_pred: str, *args, **kwargs) -> str:
        test = assert_pred
        return test
...
if __name__=='__main__':
    ...
    generator = None
    if args.generator == "toga" or args.generator == "togll":
        generator = TogaGenerator()
    elif args.generator == "naive":
        generator = NaiveGenerator()
    elif args.generator == "togs" or args.generator[:4] == "togs":
        generator = TogsGenerator()
