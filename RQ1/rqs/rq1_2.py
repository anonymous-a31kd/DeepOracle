...
def read_result_df(data_dir: str, src: str, idx: int, system: str, result_dir: str):
    ...
    system_name = system
    if system.startswith("deeporacle") or system.startswith("llm_direct"):
        system_name = "togs"
    result_file = os.path.join(data_dir, f'{system}/{system_name}_generated/{result_dir}/full_test_data.csv')
    print(result_file)
...
def cal_result(data_dir="../data/d4j_evo_prefix", 
               system = 'deeporacle'):
    scorers = OrderedDict({
        "BugFound": BugFound(),
        "FPRate": FPR(),
    ...
    for src in src_names:
        for result_dir in ["results", "merged_results"]:
            exp_results = []
            for i in range(1, 2):
                result = cal_one_result(data_dir, src, 1, system, result_dir, scorers)
    ...
    if system.startswith("deeporacle") or system.startswith("llm_direct"):
        system_name = "togs"
    df.to_csv(f'{data_dir}/{system}/{system_name}_generated/results/rq1_2.csv')
    df.to_csv(f'{data_dir}/{system}/rq1.csv')
